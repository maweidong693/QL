    ...
    public boolean updateMessageRevoked(String msgId) {
        for (int i = 0; i < mDataSource.size(); i++) {
            ...
            if (messageInfo.getId().equals(msgId)) {
                ...
                if (MessageInfoUtil.checkMsgIsQuote(messageInfo.getQuoteMsg())){
                    Gson gson = new Gson();
                    MessageQuoteInfo quoteInfo = gson.fromJson(messageInfo.getQuoteMsg(), MessageQuoteInfo.class);
                    if (msgId.equals(quoteInfo.getId())){
                        updateAdapter(MessageLayout.DATA_CHANGE_TYPE_UPDATE, i);
                    }
                }
