...
public class MessageForwardListView extends LinearLayout {

    private static final String TAG = MessageForwardListView.class.getSimpleName();
    ...
    private List<ContactItemBean> mGroupData = new ArrayList<>();
    ...
    public MessageForwardListView(Context context) {
        super(context);
        init();
    }

    public MessageForwardListView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public MessageForwardListView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        mAdapter = new MessageForwardAdapter(mData);
        ...
        loadDataSource(MessageForwardListView.DataSource.FRIEND_LIST);
        mAdapter.notifyDataSetChanged();
    }

    public MessageForwardAdapter getAdapter() {
        return mAdapter;
    }
...
//                mData.clear();
                ...
                for (V2TIMGroupInfo info : v2TIMGroupInfos) {
                    ...
                    mGroupData.add(bean.covertTIMGroupBaseInfo(info));
                }
                if (mData.isEmpty()){
                    mData.addAll(mGroupData);
                }else {
                    mData.addAll(0, mGroupData);
                }
                setDataSource(mData);
            }
        });*/
